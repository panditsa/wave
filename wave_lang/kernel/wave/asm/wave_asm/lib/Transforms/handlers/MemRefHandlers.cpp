...
LogicalResult handleMemRefLoad(Operation *op, TranslationContext &ctx) {
  ...
  if (isLDSMemRef(memrefType)) {
    ...
    // Global load
    if (ctx.getPendingSRDBaseAdjust(loadOp.getMemRef())) {
      return op->emitError(
          "pending dynamic SRD base adjustment is not supported in "
          "memref.load; lower through vector.load/store path");
    }
...
LogicalResult handleMemRefStore(Operation *op, TranslationContext &ctx) {
  ...
  if (isLDSMemRef(memrefType)) {
    ...
    // Global store
    if (ctx.getPendingSRDBaseAdjust(storeOp.getMemRef())) {
      return op->emitError(
          "pending dynamic SRD base adjustment is not supported in "
          "memref.store; lower through vector.load/store path");
    }
    auto sregType = ctx.createSRegType(4, 4);
    auto srd = PrecoloredSRegOp::create(builder, loc, sregType, 8, 4);
