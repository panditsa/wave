...
def get_tagged_mxfp4_gemm_preshuffle_bdata(
    shape: tuple[int, int, int] = (1024, 1024, 8192),
    block_shape: tuple[int, int, int] = (256, 256, 256),
    wave_shape: tuple[int, int] = (2, 2),
    mfma_variant: ScaledMMAType = ScaledMMAType.F32_16x16x128_F8F6F4,
    ...
    """Return a tagged MXFP4 scaled GEMM kernel with preshuffled B data only.

    B data is read directly from global memory using a preshuffle mapping
    (aiter shuffle_weight permutation).  B scales are read from global memory
    without any preshuffle mapping.  A and A_scale go through shared memory
    (LDS) as usual.
...
def get_tagged_mxfp4_gemm_preshuffle_bscale(
    shape: tuple[int, int, int] = (1024, 1024, 8192),
    block_shape: tuple[int, int, int] = (256, 256, 256),
    wave_shape: tuple[int, int] = (2, 2),
    mfma_variant: ScaledMMAType = ScaledMMAType.F32_16x16x128_F8F6F4,
    ...
    """Return a tagged MXFP4 scaled GEMM kernel with preshuffled B_scale only.

    B scales are read from global memory using an e8m0 scale preshuffle mapping.
    A and B data go through shared memory (LDS) as usual.
