...
class MultiDeviceLaunchable:
    ...
    def __call__(
        ...
        """
        # Infer device from inputs if needed
        inferred_device = None
        if not self._devices and device is None:
            for arg in args:
        ...
        for arg in args:
            if isinstance(arg, Tensor):
                ...
                # Use first device for imports (runtime handles distribution)
        ...
        for i in range(len(ret_list)):
            ...
            if isinstance(result, VmRef):
                ...
                if buffer_view is not None:
                    torch_tensor = self._export_torch_tensor(buffer_view, device_obj)
    ...
    def from_vmfb(
        ...
        """
        return cls(vmfb_path, devices, function_name, runtime_flags)
