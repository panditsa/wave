...
with f8e8m0fnu scales, with advanced buffering strategies and wave staggering.

Two buffering strategies are demonstrated:
1. Triple buffering (3-stage pipeline) - test_gfx1250_mxfp_gemm
...
2. Double buffering (2-stage pipeline) - test_gfx1250_mxfp_gemm_double_buffering
...
5. Partitioning by K dimension to interleave compute with memory ops
"""
...
def test_gfx1250_mxfp_gemm(is_debug=False, shape=(1024, 1024, 8192), block=(256, 256, 256)):
    """
    ...
    def mxfp_tbuf_schedule():
        ...
        prologue_clusters = [
            tkw.cluster(
                [
                    prologue_global_to_shared,
                    tkw.TensorCounterWait(4),  # Fully wait for prefetch
        ...
        clusters = [
            tkw.cluster(
                [
                    ...
                    loop_shared_load_a,
                    loop_shared_load_b,
                    loop_shared_load_a_scale,
                    loop_shared_load_b_scale,
...
def test_gfx1250_mxfp_gemm_double_buffering(is_debug=False, shape=(1024, 1024, 8192), block=(256, 256, 256)):
    """
    GFX1250 scaled WMMA GEMM with MXFP4 data type and double buffering (2-stage pipeline).
    ...
    - Double buffering: 1 prefetch iteration before the loop
    - Double buffering uses less shared memory but has less overlap
    ...
    # Define the 2-stage double-buffering schedule with wave staggering
    @wave_schedule.wave_schedule()
    def mxfp_double_buf_schedule():
        """
        2-stage double-buffering pipeline for MXFP4 scaled GEMM with wave staggering.
        
        - Prologue: Prefetch 1 iteration worth of data
        - Main loop:
          * Stage 0: Async global-to-shared prefetch for next iteration
          * Stage 1: Load from shared memory + Compute current data (overlapped)
        
        This is simpler than triple buffering and uses less shared memory,
        but provides less memory/compute overlap.
        """
        # Get the k loop to pipeline
        k_loop = tkw.get_node_by_tag("k_loop")
        ...
        # Partition MMAs and shared loads by K dimension for interleaving
        ...
        # Double buffering cluster structure (similar to 6.2's async_gemm_schedule)
        clusters = [
            tkw.cluster(
                [
                    # First half of shared loads
            ...
            tkw.cluster(
                [
                    # Reshapes and bitcasts for first half
                    loop_reshape_a,
                    loop_reshape_b,
                    loop_bitcast_a,
                    loop_bitcast_b,
                    loop_bitcast_a_scale,
                    loop_bitcast_b_scale,
                    # First half of MMAs with high priority
                    tkw.SetWavePrio(1),
                    loop_scaled_mma_0,
                    tkw.SetWavePrio(0),
                    tkw.SchedulingBarrier([]),
            ...
            tkw.cluster(
                [
                    # Second half of MMAs with high priority
                    tkw.SetWavePrio(1),
                    loop_scaled_mma_1,
                    tkw.SetWavePrio(0),
                    tkw.SchedulingBarrier([]),
    ...
    with open("gemm_mxfp4_double_buf.mlir", "w") as f:
        f.write(gemm.asm)
    ...
    print("GFX1250 MXFP GEMM double buffering test passed!")
