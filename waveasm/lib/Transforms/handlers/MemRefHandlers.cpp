...
//   - memref.extract_strided_metadata - propagate base/offset from source
//
//===----------------------------------------------------------------------===//
...
/// Handle memref.extract_strided_metadata.
///
/// This op is generated by _linearize_memref in read_write.py to extract the
/// dynamic offset from a typed memref before reinterpreting it as a 1-D flat
/// buffer. The assembler only needs result[1] (offset); the base buffer (result
/// [0]) maps to the source SRD, and the sizes/strides are unused in our
/// codegen paths.
///
/// Pattern emitted by the Python compiler:
///   %2d = memref.reinterpret_cast %arg to offset: [%c0], sizes: [%M, K],...
///   %base, %off, ... = memref.extract_strided_metadata %2d
///   %1d = memref.reinterpret_cast %arg to offset: [%off], sizes: [max],...
///
/// We map:
///   result[0] (base buffer) -> the SRD of %2d
///   result[1] (offset)      -> the mapped value of %c0 (the dynamic offset
///                              operand of the reinterpret_cast that produced
///                              %2d), or a constant 0 if the offset is static
///   result[2+] (sizes/strides) -> unmapped (not needed by downstream code)
LogicalResult handleMemRefExtractStridedMetadata(Operation *op,
                                                 TranslationContext &ctx) {
  auto &builder = ctx.getBuilder();
  auto loc = op->getLoc();
  auto extractOp = cast<memref::ExtractStridedMetadataOp>(op);
  Value source = extractOp.getSource();

  // Map base buffer result to the same value as the source memref.
  if (auto srcMapped = ctx.getMapper().getMapped(source))
    ctx.getMapper().mapValue(extractOp.getBaseBuffer(), *srcMapped);

  // Map the offset result: walk back through reinterpret_cast to find the
  // actual dynamic offset operand, then look it up in the value map.
  Value offsetVal;
  if (auto castOp = source.getDefiningOp<memref::ReinterpretCastOp>()) {
    auto staticOffsets = castOp.getStaticOffsets();
    bool hasDynamicOffset = !staticOffsets.empty() &&
                            staticOffsets[0] == ShapedType::kDynamic &&
                            !castOp.getOffsets().empty();
    if (hasDynamicOffset) {
      if (auto mapped = ctx.getMapper().getMapped(castOp.getOffsets()[0]))
        offsetVal = *mapped;
    }
  }

  if (!offsetVal) {
    // Static zero offset (or source not a reinterpret_cast): emit constant 0.
    auto immType = ctx.createImmType(0);
    offsetVal = ConstantOp::create(builder, loc, immType, 0);
  }

  ctx.getMapper().mapValue(extractOp.getOffset(), offsetVal);

  // Sizes and strides results are not used by any downstream WaveASM handler,
  // so we leave them unmapped.
  return success();
}
