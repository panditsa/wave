...
LogicalResult handleArithConstant(Operation *op, TranslationContext &ctx) {
  ...
  // For non-splat constants (e.g., iota [0,1,...,N-1] used in buffer_ops
  // address computation), map to element 0 since vector.extract at
  // position 0 will select this thread's value.
  if (auto denseAttr = dyn_cast<DenseElementsAttr>(value)) {
    if (denseAttr.isSplat()) {
      ...
      // Non-splat vector: extract element 0 for this thread's lane value.
      auto firstVal = *denseAttr.value_begin<Attribute>();
      if (auto intAttr = dyn_cast<IntegerAttr>(firstVal)) {
        int64_t intVal = intAttr.getInt();
        auto immType = ctx.createImmType(intVal);
        auto immOp = ConstantOp::create(builder, loc, immType, intVal);
        ctx.getMapper().mapValue(constOp.getResult(), immOp);
