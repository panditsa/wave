...
def classify_instruction(line: str) -> Optional[str]:
    ...
    if mnemonic.startswith("ds_"):
        return "ds"
    if mnemonic.startswith("buffer_") or mnemonic.startswith("global_") or mnemonic.startswith("flat_"):
        return "vmem"
...
def format_comparison(backends: dict, pcfg: Optional[ParityConfig] = None) -> str:
    ...
    if any(ls is not None and ls.loop_body_instructions > 0 for ls in loop_list):
        ...
        lines.append(lrow("Loop body instructions", lambda ls: ls.loop_body_instructions))
        ...
        lines.append(lrow("Prefetch loads before MFMA", lambda ls: ls.prefetch_loads_before_first_mfma))
        lines.append(lrow("waitcnt/barrier ratio", lambda ls: ls.waitcnt_per_barrier, ".2f"))
        lines.append("=" * w)
    ...
    with open(csv_path, "r") as f:
        ...
        for row in reader:
            ...
            if not inst or inst.startswith(";"):
                continue
            entries.append(ATTInstructionEntry(
...
def analyze_att_trace(
    ...
    for e in entries:
        ...
        analysis.idle_by_category[cat] = (
            analysis.idle_by_category.get(cat, 0) + e.idle)
        ...
        if cost > threshold:
            ...
            if current_region is not None and (
                ...
                if not current_region.dominant_instruction:
                    ...
                    current_region.category = _classify_att_instruction(max_e.instruction)
    ...
    if current_region is not None and (
        ...
        if not current_region.dominant_instruction and current_region.instructions:
            ...
            current_region.category = _classify_att_instruction(current_region.instructions[0])
        analysis.stall_regions.append(current_region)
...
def format_att_analysis(analyses: List[ATTAnalysis]) -> str:
    ...
    lines.append(row("Useful compute (MFMA/total)", lambda a: a.useful_compute_pct, ".1%"))
    lines.append("")
    ...
    for a in analyses:
        if a.stall_regions:
            lines.append("-" * w)
            lines.append(f"High-Cost Regions: {a.backend_name} "
            ...
            for i, region in enumerate(a.stall_regions[:8]):
                ...
                lines.append(
                    f"    Dominant: {region.dominant_instruction[:80]}")
                lines.append(
...
def _write_trace_script_llvm(
    path: Path, M: int, N: int, K: int,
    block_shape: Tuple[int, int, int], num_waves: int,
) -> None:
...
def capture_att_trace(
    ...
    try:
        ...
        if proc.returncode != 0:
            print(f"  ATT trace: process exited {proc.returncode} "
        ...
        if non_empty:
            ...
            if raw_att_files:
                ...
                print(f"  Raw ATT data captured: {len(raw_att_files)} .att files "
                      f"({total_raw:,} bytes total)")
                print(f"  NOTE: The ATT decoder produced empty CSVs but raw "
                      f"trace data is available in the output directory.")
                print(f"  You can decode these traces with the UI tool or "
                      f"re-run the ATT decoder manually.")
                return True
            else:
                print(f"  WARNING: No trace data captured. "
...
def main():
    parser = argparse.ArgumentParser(description="MXFP4 parity benchmark: aiter vs wave")
    parser.add_argument("--output-dir", type=str, default="/tmp/mxfp4_bench",
                        help="Directory to dump assembly files")
    parser.add_argument("--warmup", type=int, default=5)
    parser.add_argument("--iters", type=int, default=20)
    parser.add_argument("--shape", type=str, default="1024,1024,8192",
                        help="M,N,K shape")
    ...
    parser.add_argument("--att-library-path", type=str,
    ...
    parser.add_argument("--rocprofv3", type=str, default=None,
    ...
    if args.check_correctness:
        ...
        print(f"  Reference output: {torch_ref.shape}, "
    ...
    try:
        ...
        w_q, w_scales = quant_func(w_fp, shuffle=True)
        w_q = shuffle_weight(w_q)
        del x_fp, w_fp; torch.cuda.empty_cache()
        ...
        if aiter_asm:
            ...
            print(f"  Assembly dumped: {asm_path} ({len(aiter_asm.splitlines())} lines)")
            print(f"  Resources: VGPRs={aiter_resources['vgprs']}, "
        ...
        del x_q, w_q, x_scales, w_scales, out; torch.cuda.empty_cache()
    except Exception as e:
        print(f"  ERROR: {e}")
    ...
    try:
        ...
        if rocmasm_files:
            ...
            print(f"  Got LLVM assembly from .rocmasm ({len(llvm_asm.splitlines())} lines)")
        else:
            ...
            if hsaco_path and Path(hsaco_path).exists():
                ...
                print(f"  Got LLVM assembly from HSACO disassembly ({len(llvm_asm.splitlines())} lines)")
        ...
        if llvm_asm:
            ...
            print(f"  VGPRs={llvm_resources.get('vgprs', 0)}, "
    ...
    try:
        ...
        from compare_backends import extract_func_from_stream_mlir, get_default_arch
        ...
        if not cpp_result.success:
            ...
            print(f"  Assembly dumped: {cpp_asm_path} ({len(cpp_asm.splitlines())} lines)")

            cpp_metrics = compute_metrics(cpp_asm)
            cpp_resources = extract_resource_usage(cpp_asm)
            print(f"  VGPRs={cpp_resources.get('vgprs', 0)}, "
            ...
            del x, w, x_scales, w_scales, c; torch.cuda.empty_cache()
    except Exception as e:
        import traceback; traceback.print_exc()
