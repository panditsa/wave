...
def test_gather_to_shared_wave_tile_aligned_coalescing():
    ...
    # CHECK-DAG:      #{{.*}} = affine_map<()[s0, s1] -> (s1 * 16 + (s0 floordiv 64) * 8 - ((s1 * 2 + s0 floordiv 64) floordiv 4) * 32)>
    # CHECK-DAG:      #{{.*}} = affine_map<()[s0, s1, s2] -> (s1 * 16 + s2 * 32 + (s0 floordiv 64) * 8 + (s0 mod 64) floordiv 8 - ((s1 * 16 + (s0 floordiv 64) * 8 + (s0 mod 64) floordiv 8) floordiv 32) * 32)>
    # CHECK-DAG:      #{{.*}} = affine_map<()[s0, s1] -> (s0 + s1 * 16 - (s0 floordiv 16) * 16)>
    # CHECK:          func.func @gemm
    # CHECK:            %[[TIDX:.+]] = gpu.thread_id  x
    # CHECK:            %[[TIDY:.+]] = gpu.thread_id  y
    # CHECK:            affine.apply #{{.*}}()[%[[TIDX]], %[[TIDY]]]
    # CHECK:            affine.apply #{{.*}}()[%[[TIDX]], %[[TIDY]], %{{.*}}]
    # CHECK:            %[[TH_OFFSET:.+]] = affine.apply #{{.*}}()[%[[TIDX]], %[[TIDY]]]
    # CHECK:            scf.for %[[IND_VAR:.+]] = %c0
    # CHECK:              amdgpu.lds_barrier
    # CHECK:              affine.apply #{{.*}}()[%[[IND_VAR]], %[[TIDX]]]
    # CHECK:              %{{.*}} = arith.addi %{{.*}}, %{{.*}} overflow<nsw> : index
